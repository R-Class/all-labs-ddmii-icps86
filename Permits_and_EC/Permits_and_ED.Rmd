...
x<- dat@data
x$id <- c(1:nrow(x)) #correcting the id
dim(x)
#new dat file has 16845
...
df.Ign <- read.csv("https://raw.githubusercontent.com/R-Class/all-labs-ddmii-icps86/master/Permits_and_EC/Permits.csv", stringsAsFactors = F)

#making dat a spatial object
coo <- dat[ ,c( "lon", "lat") ]
dat <- SpatialPointsDataFrame(coo, dat, proj4string=CRS("+proj=longlat +datum=WGS84")) #this is so that it does not lose the lat lon in the dataframe when transformed into a sp
